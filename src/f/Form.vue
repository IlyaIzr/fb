...
export default {
  ...
  methods: {
    ...
    drawRows() {
      let res = [];
      fbGlobal.fields = fbGlobal.fields || {};
      const self = this;
      ...
      Object.entries(fbGlobal.fields).forEach(([key, config]) => {
        Object.defineProperty(fbGlobal.fields, key, {
          ...
          set(conf) {
            ...
            this["_" + key] = { ...this["_" + key], ...conf };
            console.log("reconfigured");
            console.log(self.fbGlobal?.fields?.["_" + key]);
            self.redrawRows()
          },
        });
        // init config setting
      ...
      res = fieldsToRows(fbGlobal.fields, this.settings.values);
      return res;
    },
    redrawRows(){
      const res = fieldsToRows(fbGlobal.fields, this.settings.values);      
      this.rows = res
